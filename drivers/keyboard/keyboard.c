...
static char scancode_to_ascii[] = { 0,
				    ESCAPE,
				    ONE,
				    TWO,
				    THREE,
...
static char scancode_to_ascii_uppercase[] = { 0,
					      ESCAPE,
					      '!',
					      '@',
					      '#',
...
#ifdef DEBUG
					log_success(
						"[keyboard]: keyboard self-test passed\n");
#endif
				} else {
					log_error(
						"[keyboard]: keyboard self-test result: ");
					printf("0x%x\n", result);
					halt();
				}
			}
...
void keyboard_interrupt_handler(struct interrupt_context *regs) {
	...
	if (!is_release && !was_extended && base < sizeof(scancode_to_ascii)) {
		char c = shift_pressed ? scancode_to_ascii_uppercase[base] :
...
struct key_event_t keyboard_multilevel_queue[KEYBOARD_QUEUE_LEVELS]
					    [RING_BUFFER_SIZE];
