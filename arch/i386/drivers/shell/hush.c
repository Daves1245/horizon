...
static struct hush_command builtin_help = { "help", "show available commands",
					    cmd_help };
static struct hush_command builtin_echo = { "echo", "echo", cmd_echo };
static struct hush_command builtin_clear = { "clear", "clear the screen",
					     cmd_clear };
static struct hush_command builtin_paging_test = { "test-paging",
						   "run the paging tests",
						   paging_test };
static struct hush_command builtin_vm_test = { "test-vm", "run the vm tests",
					       vm_test };
// TODO update to use console & graphics
// static struct hush_command builtin_maze = {"maze", "generate maze", cmd_maze};
static struct hush_command builtin_display = { "display",
					       "display the current maze",
					       cmd_display };
static struct hush_command builtin_solve = { "solve", "solve the current maze",
					     cmd_solve };
static struct hush_command builtin_pong = { "pong", "play pong", cmd_pong };
static struct hush_command builtin_peek = { "peek",
					    "hexdump memory: peek <addr> [len]",
					    cmd_peek };
...
struct hush_command const *hush_lookup_registry(const char *name) {
	...
	for (int i = 0; i < num_commands; i++) {
		...
		printf("[DEBUG] Comparing with registry[%d]: '%s' (len=%zu vs %zu)\n",
		       i, hush_registry[i]->name, name_len,
		       strnlen(hush_registry[i]->name, COMMAND_NAME_LEN));
#endif
...
void hush_init() {
	...
	while (hush_state.running) {
		...
		if (len > 0) {
			int copy_len = len < MAX_COMMAND_LEN - 1 ?
					       len :
					       MAX_COMMAND_LEN - 1;
...
enum HUSH_STATE hush_execute_command() {
	...
	struct hush_command const *command =
		hush_lookup_registry(hush_state.name);
	if (!command) {
